...
export default function ButtonLinkCard({ styles, data }){
  ...
  return(
    <section className={ [styles[styleString], 'w-full flex flex-col items-center text-center self-center py-12 px-8'].join(' ') }>
      ...
      { data.description && <h4 className="text-lg sm:text-xl font-semibold">{ data.description }</h4> }
      <a href={ data.link.path } className="mt-8 py-2 px-4 text-xl font-semibold rounded-md">{ data.link.title }</a>
    </section>
  );
