...
export default function Hero({ styles, globalStyles, info }){
  ...
  return (
    <section className={ [styles.hero, 'w-full relative lg:h-96 lg:overflow-hidden'].join(' ') }>
      ...
      <article className={[styles.message, "w-full h-64  px-8 lg:h-full lg:absolute lg:z-20 lg:top-0 lg:pe-96"].join(' ') }>
        <div className="h-full flex flex-col items-start justify-evenly lg:w-96 ms-20">
          ...
          <button className="rounded-md w-48 h-12 text-2xl self-center">Get Started</button>
        </div>
      </article>
    </section>
  );
